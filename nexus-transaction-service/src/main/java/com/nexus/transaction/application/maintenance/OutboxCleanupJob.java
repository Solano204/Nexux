package com.nexus.transaction.application.maintenance;
 ...
 *
 * Requires @EnableScheduling, which nexus-transaction-service did not have
 * anywhere until this session (see config.ObservabilityConfig) - without
 * it this class's @Scheduled annotation would compile fine and never
 * fire, the same silent-no-op failure mode as TransactionSagaOrchestrator's
 * two timeout jobs.
 */
@Slf4j
@Component
public class OutboxCleanupJob {
    ...
    public OutboxCleanupJob(OutboxRepository outboxRepository,
        ...
        this.deletedCounter = Counter.builder("transaction.outbox.cleanup.deleted.total")
                .description("Outbox rows deleted by the retention cleanup job")
                .register(meterRegistry);
    }

    @Scheduled(cron = "0 15 2 * * *", zone = "America/Mexico_City")
    @Transactional
