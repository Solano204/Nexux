...
public class TransactionCommandService {
    ...
    public void processLedgerResult(UUID transactionId, UUID sagaId, boolean success, UUID ledgerEntryId, String failureReason, String traceId) {
        Transaction txn = loadTransaction(transactionId);
        if (success) {
            txn.markLedgerPosted(ledgerEntryId); txn.markCompleted(); transactionRepository.save(txn);
            writeOutbox(transactionId, sagaId, "TransactionCompleted", buildCompletedPayload(txn, traceId));
            searchIndexer.indexAsync(txn); completedCounter.increment();
        } else {
            txn.markLedgerFailed(failureReason); transactionRepository.save(txn);
            writeOutbox(transactionId, sagaId, "BalanceReleaseRequested", buildBalanceReleasePayload(txn, traceId));
            writeOutbox(transactionId, sagaId, "TransactionFailed", buildFailedPayload(txn, failureReason, traceId));
            searchIndexer.indexAsync(txn); failedCounter.increment();
        }
    }
    ...
    private void writeOutbox(UUID aggregateId, UUID sagaId, String eventType, ObjectNode payload) { payload.put("sagaId", sagaId.toString()); outboxRepository.save(OutboxEntry.of("TRANSACTION", aggregateId, eventType, payload)); }
    ...
    private ObjectNode buildLedgerPostingPayload(Transaction t, String traceId) { return objectMapper.createObjectNode().put("transactionId",t.getTransactionId().toString()).put("sourceAccountId",t.getSourceAccountId().toString()).put("targetAccountId",t.getTargetAccountId()!=null?t.getTargetAccountId().toString():null).put("amount",t.getAmount().toPlainString()).put("currency",t.getCurrency()).put("transactionType",t.getTransactionType().name()).put("traceId",traceId); }
    ...
    private ObjectNode buildCompletedPayload(Transaction t, String traceId) { return objectMapper.createObjectNode().put("transactionId",t.getTransactionId().toString()).put("userId",t.getUserId().toString()).put("sourceAccountId",t.getSourceAccountId().toString()).put("targetAccountId",t.getTargetAccountId()!=null?t.getTargetAccountId().toString():null).put("amount",t.getAmount().toPlainString()).put("currency",t.getCurrency()).put("ledgerEntryId",t.getLedgerEntryId().toString()).put("completedAt",Instant.now().toString()).put("traceId",traceId); }
