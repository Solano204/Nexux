...
/**
 * Saga Reply Consumer — Handles balance reservation replies.
 *
 * Subscribes to: saga.replies
 * Handles: BalanceReservedReply, BalanceReservationFailedReply
 *
 * Bridges the Account Service's SAGA replies back into
 * the Transaction Service's choreography flow.
 */
@Slf4j
@Component
...
public class SagaReplyConsumer {
    ...
    @KafkaListener(
            topics = "saga.replies",
            groupId = "transaction-service-saga-replies",
    ...
    public void consumeSagaReply(String message, Acknowledgment ack) {
        ...
        Observation obs = Observation.createNotStarted(
                ...
                .lowCardinalityKeyValue("topic", "saga.replies")
                .start();
        ...
        try {
            ...
            // Only process balance-related replies
            if (!replyType.startsWith("Balance")) {
                ack.acknowledge();
                return;
            }

            String sourceService = reply.path("sourceService").asText();
            if (!"nexus-account-service".equals(sourceService)) {
                ack.acknowledge();
                return;
            }

            UUID transactionId = UUID.fromString(
                    reply.path("transactionId").asText());
            ...
            String failureReason = !success
                    ? reply.path("payload").path("reason").asText()
                    : null;

            log.info("SAGA balance reply: type={} txnId={} success={}",
                    replyType, transactionId, success);

            commandService.processBalanceResult(
                    transactionId, sagaId, success,
                    failureReason, traceId);

            ack.acknowledge();
            obs.event(Observation.Event.of(
                    "saga.reply." + replyType + ".processed"));
