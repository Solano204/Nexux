 ...
 *    - Section 3 structured output: entity(RiskScoringPlan.class)
 *
 * 2. riskExecutionClient
 *    - gpt-4o, temp=0.0, JSON, ALL 8 tools registered
 *    - internalToolExecutionEnabled=false: WE drive the loop
 *    - Section 11 Plan-then-Act pattern
 *    - Named Observation per tool call → Zipkin child span
 *
 * Temperature 0.0 on BOTH: risk scores must be deterministic.
 * Same input data → same risk score. Required for compliance.
 */
@Configuration
