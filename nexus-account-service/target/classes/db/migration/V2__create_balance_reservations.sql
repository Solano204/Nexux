CREATE TABLE balance_reservations (
    ...
    status          VARCHAR(25)     NOT NULL DEFAULT 'ACTIVE'
                    CHECK (status IN ('ACTIVE','RELEASED','FINALIZED','RELEASED_BY_EXPIRY')),
    ...
    CONSTRAINT uq_active_reservation UNIQUE (account_id, transaction_id)
);
...
CREATE INDEX idx_reservations_expires ON balance_reservations (expires_at) WHERE status = 'ACTIVE';
CREATE INDEX idx_reservations_transaction ON balance_reservations (transaction_id);