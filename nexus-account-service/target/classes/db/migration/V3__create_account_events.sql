...
CREATE INDEX idx_account_events_occurred ON account_events (account_id, occurred_at DESC);
CREATE INDEX idx_account_events_type ON account_events (event_type);
CREATE INDEX idx_account_events_transaction ON account_events (transaction_id) WHERE transaction_id IS NOT NULL;
CREATE OR REPLACE FUNCTION prevent_account_event_modification()
RETURNS TRIGGER AS $$ BEGIN RAISE EXCEPTION 'account_events are immutable: %', TG_OP; END; $$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_account_events_immutable
    BEFORE UPDATE OR DELETE ON account_events FOR EACH ROW EXECUTE FUNCTION prevent_account_event_modification();