CREATE TABLE accounts (
    ...
    total_balance               DECIMAL(20,4)   GENERATED ALWAYS AS (available_balance + reserved_amount) STORED,
    ...
    CONSTRAINT chk_reserved_amount_non_negative   CHECK (reserved_amount >= 0),
    CONSTRAINT chk_daily_used_non_negative        CHECK (daily_transaction_used >= 0),
    CONSTRAINT chk_monthly_used_non_negative      CHECK (monthly_transaction_used >= 0)
);
CREATE UNIQUE INDEX uq_accounts_number ON accounts (account_number);
CREATE INDEX idx_accounts_user_id ON accounts (user_id);
CREATE INDEX idx_accounts_status ON accounts (status);
CREATE INDEX idx_accounts_user_status ON accounts (user_id, status) WHERE status != 'CLOSED';
...
CREATE TRIGGER trigger_accounts_updated_at
    BEFORE UPDATE ON accounts FOR EACH ROW EXECUTE FUNCTION accounts_update_updated_at();