...
class JwtValidatorTest {
    ...
    // ObservationRegistry.NOOP, not a @Mock: Observation.createNotStarted()
    // calls real methods (observationConfig().isObservationEnabled(...))
    // internally - an unstubbed Mockito mock returns null there and NPEs.
    // NOOP is Micrometer's real no-op implementation, safe to use directly.
    final ObservationRegistry observationRegistry = ObservationRegistry.NOOP;

    LocalJwtValidator jwtValidator;

    @BeforeEach
    void setUpValidator() {
        jwtValidator = new LocalJwtValidator(jwksCache, observationRegistry);
    }
