...
#
# Spring Boot's own ${FOO:?required} properties expect a plain env var
# though, not a file path — so for every FOO_FILE=/run/secrets/<name> this
# container was given, read the file and export FOO=<contents> before
# handing off to java. `exec "$@"` at the end replaces this script's PID
# with the JVM's, so java still ends up as PID 1 and still receives
# SIGTERM directly — the same guarantee plain `ENTRYPOINT ["java", ...]`
# had, just with one extra step before the handoff.
set -e
...
# non-secret vars). No-op for services that don't set MONGO_DB.
