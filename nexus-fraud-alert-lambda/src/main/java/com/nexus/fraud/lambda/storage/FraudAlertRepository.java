...
public class FraudAlertRepository {
    ...
    public String storeAlert(FraudAlertEvent alert,
        ...
        // Fraud decision as JSON (preserves full nested structure)
        ...
        try {
            dynamo.putItem(PutItemRequest.builder()
                    .tableName(alertsTable)
                    .item(item)
                    // Idempotent write: reject if PK already exists
                    .conditionExpression("attribute_not_exists(PK)")
                    .build());
            ...
            // Alert already stored from a previous Lambda execution
    ...
    public void createSarConsideration(FraudAlertEvent alert,
        ...
        Map<String, AttributeValue> sarItem = Map.of(
                "PK", av("SAR#" + sar.sarId()),
                "SK", av("METADATA"),
                "sarId", av(sar.sarId()),
                "alertId", av(alertId),
                "transactionId", av(alert.transactionId()),
                "userId", av(alert.userId()),
                "considerationReason", av(sar.reason()),
                "patternType", av(sar.patternType()),
                "sarThresholdMet", AttributeValue.fromBool(true),
                "automaticConsideration", AttributeValue.fromBool(true),
                "status", av("PENDING_COMPLIANCE_REVIEW"),
                "regulatoryDeadline", av(deadline.toString()),
                "createdAt", av(now.toString()),
                "ttl", avn(String.valueOf(ttl))
        );
