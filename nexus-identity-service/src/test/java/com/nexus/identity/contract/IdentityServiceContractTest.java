...
class IdentityServiceContractTest {
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("nexus.plane-bridge-secret", () -> "test-bridge-secret");
    }
    ...
    void internalKycStatus_responseContractFieldsPresent() throws Exception {
        ...
        // Requires X-Plane-Bridge-Secret since 13_REST_API_DESIGN_CHANGES.md's
        // audit - auth-lambda calls this from outside the Docker network, so
        // RemoteAddr alone isn't a real boundary. Test secret configured via
        // DynamicPropertySource above.
        mockMvc.perform(get("/internal/v1/users/{userId}/kyc/status", userId)
                        .header("X-Plane-Bridge-Secret", "test-bridge-secret"))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("CONTRACT: /internal/v1/users/{id}/kyc/status rejects missing bridge secret")
    void internalKycStatus_missingBridgeSecret_returns401() throws Exception {
        Map<String, Object> body = Map.of(
                "email", "internal-kyc-noauth@nexusbank.com",
                "password", "SecurePassword123!",
                "fullName", "Internal KYC No Auth Test",
                "phoneNumber", "+525512341015",
                "dateOfBirth", "1990-01-15",
                "country", "MX"
        );
        String regResponse = mockMvc.perform(
        ...
        // No X-Plane-Bridge-Secret header sent - this is the actual
        // security boundary this contract test should also verify, not
        // just the happy path.
        mockMvc.perform(get("/internal/v1/users/{userId}/kyc/status", userId))
